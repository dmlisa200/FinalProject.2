...
class Calculator(ttk.Frame):
    def __init__(self, container, **kwargs):
        ...
        ttk.Label(self, text="College costs for children: ").grid(row=5, column=0, sticky=W)
        validate = (self.register(self.validate_number), "%P")
        ttk.Entry(self, validate='key', validatecommand=validate, invalidcommand=self.print_error, width=10,
                  textvariable=self.college_cost).grid(row=5, column=1, sticky=W)
        Button(self, text="Submit", command=self.college).grid(row=5, column=3, sticky=E)

        ttk.Label(self, text="Total debt, mortgage and other:  ").grid(row=6, column=0, sticky=W)
        validate = (self.register(self.validate_number), "%P")
        ttk.Entry(self, validate='key', validatecommand=validate, invalidcommand=self.print_error, width=10,
                  textvariable=self.debt).grid(row=6, column=1, sticky=W)
        Button(self, text="Submit", command=self.cap_required).grid(row=6, column=3, sticky=E)

        ttk.Label(self, text="Total saving and investments: ").grid(row=7, column=0, sticky=W)
        validate = (self.register(self.validate_number), "%P")
        ttk.Entry(self, validate='key', validatecommand=validate, invalidcommand=self.print_error, width=10,
                  textvariable=self.savings).grid(row=7, column=1, sticky=W)

        ttk.Label(self, text="Total Retirement savings: ").grid(row=8, column=0, sticky=W)
        validate = (self.register(self.validate_number), "%P")
        ttk.Entry(self, validate='key', validatecommand=validate, invalidcommand=self.print_error, width=10,
                  textvariable=self.retirement).grid(row=8, column=1, sticky=W)

        ttk.Label(self, text="Present amount of life insurance you have: ").grid(row=9, column=0, sticky=W)
        validate = (self.register(self.validate_number), "%P")
        ttk.Entry(self, validate='key', validatecommand=validate, invalidcommand=self.print_error, width=10,
                  textvariable=self.present_amount).grid(row=9, column=1, sticky=W)
        Button(self, text="Submit", command=self.assets).grid(row=9, column=3, sticky=E)

        Button(self, text="reset", command=self.restart_program).grid(row=10, column=2, sticky=E)
